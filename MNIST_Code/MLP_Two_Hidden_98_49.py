...
layerOne = []
for x in range(98):
    layerOne.append(HiddenNode(np.random.uniform(-0.2,0.2,196), np.random.ranf(1)))

layerTwo = []
for x in range(49):
	layerTwo.append(HiddenNode(np.random.uniform(-0.2,0.2,98), np.random.ranf(1)))

layerOut = []
for x in range(10):
    layerOut.append(OutputNode(np.random.uniform(-0.2,0.2,49), np.random.ranf(1)))
...
for x in range(epochs):
	...
	for i in merged_data:
		...
		for node in layerOne:
			node.net = netCalcInput(node, data)
			node.out = tanh(node.net) 
			node.weightsOld = np.array(node.weights)
			
		for node in layerTwo:
		...
		for node in layerOut:
			node.net = netCalc(node, layerTwo)
		...
		for node in layerOut:
				...
				for w in node.weights:
					node.weights[i] = w - (lRate * layerTwo[i].out * node.d + (momentum * (node.weightsOld[i] - w)))
					i += 1
		...
		for node in layerTwo:
			i = 0
			node.d = layerUpdate(layerOut, node.net, j)
			for w in node.weights:
				node.weights[i] = w - (lRate * data[i] * node.d + (momentum * (node.weightsOld[i] - w)))
				i += 1
			node.bias = node.bias - lRate * node.d
			j += 1
		...
		for node in layerOne:
			i = 0
			node.d = layerUpdate(layerTwo, node.net, j)
			for w in node.weights:
				node.weights[i] = w - (lRate * data[i] * node.d + (momentum * (node.weightsOld[i] - w)))
				i += 1
			node.bias = node.bias - lRate * node.d
